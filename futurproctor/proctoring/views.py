...
import random  # Randomization for shuffling questions
...
try:
    ...
    logging.warning("face_recognition library not available — facial recognition disabled")
...
def registration(request):
    ...
    if request.method == 'POST':  # Check if form is submitted
        ...
        try:
            # Decode the base64 image (photo_data comes in "data:image/jpeg;base64,ENCODED_DATA")
            ...
            # Extract face encoding from the image (optional, may return None)
            face_encoding = get_face_encoding(image)
            # Note: face_encoding can be None if face_recognition is unavailable or no face detected
        ...
        try:
            ...
            face_encoding_data = face_encoding.tolist() if face_encoding is not None else None
            student = Student(
                user=user,
                name=name,
                address=address,
                ...
                face_encoding=face_encoding_data,  # None if face recognition unavailable
            )
            student.save()
...
def get_face_encoding(image):
    ...
    - Returns None if no faces are detected or face_recognition is unavailable.
    """
    ...
    try:
        ...
        logging.exception("Error extracting face encoding: %s", e)
        return None
...
def match_face_encodings(captured_encoding, stored_encoding):
    ...
    try:
        ...
        logging.exception("Error comparing face encodings: %s", e)
        return False
...
def login(request):
    ...
    if request.method == "POST":
        ...
        try:
            # Decode the base64 image (remove the "data:image/jpeg;base64," prefix)
            ...
            # Extract face encoding from the captured image (optional)
            captured_encoding = get_face_encoding(image)
            # Note: captured_encoding can be None if face_recognition is unavailable or no face detected
            # In that case, we'll fall back to email/password authentication
            ...
            try:
                ...
                # If facial recognition is available and encodings exist, verify face
                if FACE_RECOGNITION_AVAILABLE and student.face_encoding and captured_encoding is not None:
                    stored_encoding = np.array(student.face_encoding)
                    # Compare the captured face encoding with the stored encoding
                    if not match_face_encodings(captured_encoding, stored_encoding):
                        return JsonResponse({"success": False, "error": "Face does not match our records."})
                    # Face matches, proceed with login
                elif not FACE_RECOGNITION_AVAILABLE and student.face_encoding is None:
                    # Face recognition unavailable and no encoding stored: login with email/password only
                    pass
                elif FACE_RECOGNITION_AVAILABLE and (student.face_encoding is None or captured_encoding is None):
                    # Face recognition available but encoding missing: fall back to email/password
                    logging.warning(f"Face encoding missing for user {email}. Using email/password auth only.")
                else:
                    # Allow login (email/password verified, facial recognition either disabled or optional)
                    pass
...
def process_frame(frame, request):
    ...
    # Check for phone or book (actual prohibited items)
    if any(label in ["cell phone", "book"] for label in detected_labels):
        ...
        cheating_event, created = CheatingEvent.objects.get_or_create(
            ...
            defaults={'cheating_flag': True, 'detected_objects': detected_labels}
        )
        # Only update if newly created to avoid duplicating detections
        if created:
            save_cheating_event(frame, request, cheating_event, detected_labels)
...
def exam(request):
    ...
    # Delete any previous cheating events for this student across all event types
    # This ensures fresh detection for each exam attempt
    CheatingEvent.objects.filter(student=student).delete()

    # Load exam questions from the JSON file
    try:
        # Use relative path based on Django project structure
        questions_file = Path(settings.BASE_DIR) / 'proctoring' / 'dummy_data' / 'ai.json'
        with open(questions_file, 'r', encoding='utf-8') as file:
            data = json.load(file)
        questions = data.get("questions", [])
        
        # Randomize questions for each exam session
        random.shuffle(questions)
        
    except FileNotFoundError as e:
        logging.error(f"Questions file not found at: {questions_file}")
        return HttpResponse(f"Error: Questions file not found! Expected at: {questions_file}", status=404)
    ...
    # Disabled: Camera is handled by frontend JavaScript
...
def submit_exam(request):
    if request.method == 'POST':
        ...
        questions = data.get('questions', [])
        correct_answers = 0
        student_answers = {}  # Store all student answers

        # Check answers based on question type
        for question in questions:
            question_id = question['id']
            user_answer = request.POST.get(f'answer_{question_id}')
            question_type = question.get('type', 'objective')
            
            # Store student answer
            student_answers[str(question_id)] = {
                'student_answer': user_answer or 'No Answer',
            ...
            if question_type == 'objective':
                # For objective questions, check exact match
                if user_answer == question['correct_answer']:
                    correct_answers += 1
                    student_answers[str(question_id)]['is_correct'] = True
            else:
                # For subjective and coding questions, they require manual review
                # We'll give default 0 points unless manually graded
                # In a full implementation, this would be reviewed by instructors
                pass

        # Create a new exam record with results
        exam = Exam.objects.create(
            student=user.student,
            exam_name='AI-Based Exam',
            total_questions=len(questions),
            correct_answers=correct_answers,
            status='completed',
            student_answers=student_answers  # Store answers
        )
...
def record_tab_switch(request):
    if request.method == "POST":
        ...
        # Get or create a CheatingEvent for the student and event type
        cheating_event, created = CheatingEvent.objects.get_or_create(
            student=student,
...
def result(request):
    ...
    # Fetch cheating events for this student
    cheating_events = CheatingEvent.objects.filter(student=user.student).order_by('timestamp')
    
    # Calculate cheating severity score (0-100) and categorize incidents
    cheating_severity = 0
    cheating_details = []
    cheating_categories = {
        'tab_switches': 0,
    ...
    for event in cheating_events:
        ...
        if event.cheating_flag:
            # Tab switch detection
            if event.tab_switch_count > 0:
                severity += min(event.tab_switch_count * 10, 40)  # Max 40 points for tab switches
                reason += f"Tab Switches: {event.tab_switch_count}. "
                cheating_categories['tab_switches'] += event.tab_switch_count
            
            # Check detected objects (e.g., phone, book, etc.)
            if event.detected_objects:
                # Only count if phone or book is actually detected
                detected_list = event.detected_objects if isinstance(event.detected_objects, list) else []
                has_phone_or_book = any(obj.lower() in ["phone", "cell phone", "book"] for obj in detected_list)
                
                if has_phone_or_book:
                    severity += 30  # 30 points for detected objects
                    reason += f"Detected Objects: {', '.join(detected_list)}. "
                    cheating_categories['objects_detected'] += 1
            
            # Event type severity
            if event.event_type == 'audio_detected':
                severity += 25
                reason += "External Audio Detected. "
                cheating_categories['audio_detected'] += 1
            elif event.event_type == 'gaze_detected':
                severity += 15
                reason += "Gaze Deviation Detected. "
                cheating_categories['gaze_issues'] += 1
            elif event.event_type == 'multiple_persons':
                severity += 35
                reason += "Multiple Persons Detected. "
                cheating_categories['multiple_persons'] += 1
        ...
        if severity > 0:
            cheating_details.append({
                ...
                'tab_switches': event.tab_switch_count,
                'detected_objects': event.detected_objects or []
            })
            cheating_severity = max(cheating_severity, severity)
    ...
    # AI-based pass/fail decision logic
    pass_fail_decision = "PASS"
    decision_reason = ""
    
    # Scoring rules
    score_percentage = percentage
    integrity_score = max(0, 100 - cheating_severity)
    
    if cheating_severity >= 70:
        # High cheating detected
        pass_fail_decision = "FAIL"
        decision_reason = f"Failed due to high cheating severity ({cheating_severity}/100). Exam integrity compromised."
    elif cheating_severity >= 50:
        # Moderate cheating
        if score_percentage < 40:
            pass_fail_decision = "FAIL"
            decision_reason = f"Failed: Low score ({score_percentage:.1f}%) + moderate cheating detected ({cheating_severity}/100)."
        else:
            pass_fail_decision = "PASS"
            decision_reason = f"Passed with warning: Cheating detected ({cheating_severity}/100) but score ({score_percentage:.1f}%) is acceptable. Manual review recommended."
    else:
        # Low or no cheating
        if score_percentage >= 40:
            pass_fail_decision = "PASS"
            decision_reason = f"Passed: Score {score_percentage:.1f}% meets pass criteria ({cheating_severity}/100 cheating severity)."
        else:
            pass_fail_decision = "FAIL"
            decision_reason = f"Failed: Score {score_percentage:.1f}% is below 40% pass threshold."

    # Calculate analytics for charts
    analytics = {
        'correct_percentage': round(percentage, 1),
        'incorrect_percentage': round(100 - percentage, 1),
        'cheating_breakdown': json.dumps(cheating_categories),
        'performance_data': json.dumps(performance_by_category)
    }

    # Aggregate incident counts for display
    total_tab_switches = cheating_categories['tab_switches']
    total_objects_detected = cheating_categories['objects_detected']
    total_audio_detected = cheating_categories['audio_detected']
    total_gaze_issues = cheating_categories['gaze_issues']
    total_multiple_persons = cheating_categories['multiple_persons']
    ...
    context = {
        ...
        'cheating_events': cheating_details,
        'cheating_severity': cheating_severity,
        'integrity_score': integrity_score,
        'pass_fail_decision': pass_fail_decision,
        'decision_reason': decision_reason,
        'questions': questions,
        'cheating_categories': cheating_categories,
        'performance_by_category': performance_by_category,
        # Individual incident counts for chart display
        'tab_switch_count': total_tab_switches,
        'phone_detection_count': total_objects_detected,
        'multiple_person_count': total_multiple_persons,
        'audio_detection_count': total_audio_detected,
        'gaze_issue_count': total_gaze_issues,
        'analytics_json': json.dumps({
            'correct': correct_answers,
            'incorrect': incorrect_answers,
            'cheating_breakdown': cheating_categories
        }),
        'exam_date': exam.timestamp.strftime('%Y-%m-%d %H:%M'),
        'student_answers': exam.student_answers if exam.student_answers else {},
    }
...
def admin_dashboard(request):
    ...
    total_cheating_events = CheatingEvent.objects.count()
    
    # Performance statistics
    all_exams = Exam.objects.all()
    avg_score = 0
    if all_exams.count() > 0:
        avg_score = sum(exam.correct_answers or 0 for exam in all_exams) / all_exams.count()
    
    # Cheating event breakdown
    cheating_breakdown = {
        'tab_switches': CheatingEvent.objects.filter(event_type='tab_switch').count(),
        'object_detection': CheatingEvent.objects.filter(event_type='object_detected').count(),
        'audio_detection': CheatingEvent.objects.filter(event_type='audio_detected').count(),
        'gaze_detected': CheatingEvent.objects.filter(event_type='gaze_detected').count(),
        'multiple_persons': CheatingEvent.objects.filter(event_type='multiple_persons').count(),
    }
    ...
    for student in students:
        ...
        # Identify high-risk students (low trust score)
        if student.trust_score < 50:
            high_risk_students.append({
                'name': student.name,
                'email': student.email,
                'trust_score': student.trust_score,
                'cheating_events': student.cheating_event_count,
                'id': student.id
            })
    ...
    for student in students:
        ...
        for exam in exams:
            score_pct = calculate_exam_score(exam)
            if score_pct == 100:
                perfect_score_students.append({
                    'name': student.name,
    ...
    context = {
        ...
        'total_cheating_events': total_cheating_events,
        'avg_score': round(avg_score, 2),
        'cheating_breakdown': cheating_breakdown,
        'high_risk_students': high_risk_students[:5],  # Top 5 high-risk
        'perfect_score_students': perfect_score_students[:5],  # Top 5 perfect scores
        'cheating_breakdown_json': json.dumps(cheating_breakdown),
    }
